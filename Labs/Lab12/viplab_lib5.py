...
def getImage_fromLUT2(band,LUT,colordefault=0):
...
def getImage_fromLUT(band,LUT,colordefault=0):
    ...
    nlut=len(LUT)    
    dataresR=np.zeros((nrows,ncols),np.uint8)
    dataresG=np.zeros((nrows,ncols),np.uint8)
    dataresB=np.zeros((nrows,ncols),np.uint8)
    
    dataresR=0
    dataresG=0
    dataresB=0

    for i in range(0,nlut):
        lutrow=LUT[i]
        c=lutrow[0]
        c2=lutrow[1]
        dataresR=np.where((band>=c) & (band<=c2),lutrow[2],dataresR)
        dataresG=np.where((band>=c) & (band<=c2),lutrow[3],dataresG)
        dataresB=np.where((band>=c) & (band<=c2),lutrow[4],dataresB)

    datares=np.dstack((dataresR,dataresG,dataresB))
