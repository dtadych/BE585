...
def Image_getOneBand(band, Band_Pos,fdiv, maxV=0):    
    nrows,ncols=band.shape
    ...
    if(maxV<=0):
      ...
      MaxV=maxV/fdiv
# datares=np.zeros((nrows,ncols,3))
    datares[:,:,Band_Pos]= band_normalize(band, fdiv,MaxV)
    ...
    def Save_Band(self,data,label=''):
        ...
        if(self.isOpen):
            ...
            if(nrows==self.nrows and ncols==self.ncols):
               ...
               print("data can not be saved because it has different rows/cols size")
        else:
            print("A file needs to be open for writting bsq")
